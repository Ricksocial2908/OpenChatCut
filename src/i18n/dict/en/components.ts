...
export default {
  ...
  '短片指令无需模型': 'Short-movie commands do not need a model',
  '排列、修剪、淡化和导出可以没有 API key。开放式 Agent 对话需要在设置里配置模型。': 'Ordering, trimming, fades, and export work without an API key. Open-ended Agent chat needs a model in Settings.',
  '用对话把多个片段剪成短片': 'Cut several clips into a short from chat',
  '新建工程，一次导入多个视频或图片，然后说「排列这些片段」「做成 30 秒短片」「导出」。这些指令不需要 API key。': 'Create a project, import several videos or images at once, then say “Order these clips”, “Make a 30s short”, or “Export”. These commands do not need an API key.',
