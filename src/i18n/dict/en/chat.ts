...
export default {
  ...
  '短片工作台': 'Short movies',
  '短片流程': 'Short movie',
  '导入片段，用对话剪成短片': 'Import clips, then cut a short in chat',
  '先放入多个视频或图片，再点一条指令。这些指令不需要 API key。更开放的要求仍交给 Agent，需要在设置里配置模型。': 'Add several videos or images, then pick a command. These commands do not need an API key. Open-ended requests still go to the Agent, which needs a model in Settings.',
  '导入多个片段': 'Import several clips',
  '对话里排列、修剪、加淡化': 'Order, trim, and fade in chat',
  '在预览里播放': 'Play the preview',
  '导出 MP4': 'Export MP4',
  '排列片段': 'Order clips',
  '按名称接到时间线': 'Place them on the timeline by name',
  '排列这些片段': 'Order these clips',
  '剪掉片头': 'Trim intros',
  '每段去掉开头 1 秒': 'Drop 1 second from the start of each clip',
  '加上淡化': 'Add fades',
  '每段淡入淡出': 'Fade each clip in and out',
  '加上淡入淡出': 'Add fades',
  '做成 30 秒': 'Make it 30 seconds',
  '收成一条短片': 'Assemble one short',
  '做成 30 秒短片': 'Make a 30s short',
  '导出': 'Export',
  '保存 MP4': 'Save an MP4',
  '导出成片': 'Export',
  '剪掉片尾': 'Trim outros',
  '静音': 'Mute clips',
  '每段最多 3 秒': 'Cut each clip to 3s',
  '排列、修剪、加淡化，或输入 / 查看短片指令': 'Order, trim, fade, or type / for short-movie commands',
  '告诉 AI 要做哪些修改 - @ 引用素材，或用 clip 2 / 文件名': 'Tell the AI what to change — @ a clip, or say clip 2 / a filename',
  '本地短片指令，无需模型 (Enter)': 'Local short-movie command, no model needed (Enter)',
  '请先在设置中配置一个模型厂商。短片指令（/order、/trim、/fades、/short、/export）无需模型。': 'Configure a model provider in Settings first. Short-movie commands (/order, /trim, /fades, /short, /export) do not need a model.',
  '短片指令与工作流': 'Short commands and workflows',
  '按名称把视频和图片接到时间线上': 'Place videos and images on the timeline by name',
  '每段去掉开头 1 秒，可写 /trim 0.5s': 'Drop 1 second from each start. /trim 0.5s changes the amount',
  '每段画面 0.5 秒淡化，可写 /fades 0.3s': 'Fade each picture for 0.5 seconds. /fades 0.3s changes the amount',
  '按名称排列并压到目标时长': 'Order by name and fit the target length',
  '关掉画面片段的原声': 'Mute the picture clips',
  '切短': 'Shorten',
  '/cut 3s 限制每段长度，/cut clip 2 删掉一段': '/cut 3s caps each clip. /cut clip 2 removes one clip',
  '打开导出窗口，选择 MP4': 'Open the export window and choose MP4',
  '短片指令': 'Short commands',
  '列出不需要模型的剪辑指令': 'List the editing commands that do not need a model',
  '（无）': '(none)',
  '请点名一个片段，例如 clip 2 或文件名。': 'Name a clip, for example clip 2 or a filename.',
  '没有第 {n} 个片段。当前有 {count} 个：{available}': 'There is no clip {n}. There are {count}: {available}',
  '找不到名为“{name}”的片段。当前可用：{available}': 'No clip named “{name}”. Available: {available}',
  '“{name}”匹配到多个片段，请写得更具体：{available}': '“{name}” matches more than one clip. Be more specific: {available}',
  '时间线上还没有画面片段。先导入，或说「排列这些片段」。': 'There are no picture clips on the timeline yet. Import some, or say “Order these clips”.',
  '“{name}”还在素材库里。先说「排列这些片段」，再剪这一段。': '“{name}” is still in the media pool. Say “Order these clips”, then edit that clip.',
  '这些短片指令在本地执行，不需要 API key。也可以直接说同样的话。': 'These short-movie commands run locally and do not need an API key. You can also say the same thing in a sentence.',
  '按名称把视频和图片接到时间线。/order a.mp4, b.mp4 按你写的顺序。': 'Place videos and images on the timeline by name. /order a.mp4, b.mp4 uses the order you write.',
  '每段剪掉开头 1 秒。/trim 0.5s、/trim outro、/trim clip 2。': 'Trim 1 second from the start of each clip. /trim 0.5s, /trim outro, /trim clip 2.',
  '每段最长 3 秒。/cut clip 2 删掉第 2 段并靠拢。': 'Cap each clip at 3 seconds. /cut clip 2 removes clip 2 and closes the gap.',
  '加上 0.5 秒淡入淡出。': 'Add a 0.5 second fade in and out.',
  '关掉原声。/unmute 恢复。': 'Mute the original audio. /unmute restores it.',
  '排列并收成大约 30 秒。': 'Order the clips and fit them to about 30 seconds.',
  '打开导出窗口，选择 MP4。': 'Open the export window and choose MP4.',
  '片段可用文件名、clip 2、第2个或最后一个。更开放的要求仍交给 Agent，需要在设置里配置模型。': 'Refer to a clip by filename, clip 2, or “last”. Open-ended requests still go to the Agent and need a model in Settings.',
  '素材库和时间线上都还没有视频或图片。先导入多个片段，再排列。': 'There are no videos or images in the pool or on the timeline. Import several clips, then order them.',
  '没有可排列的视频或图片。': 'There are no videos or images to order.',
  '时间线上还没有画面，导出会是空白。先导入并排列片段。导出窗口已打开，有成片后选择 MP4。': 'There is no picture on the timeline, so an export would be blank. Import and order clips first. The export window is open; choose MP4 once there is a cut.',
  '导出窗口已打开。当前成片约 {seconds} 秒。选择 MP4 即可保存。': 'The export window is open. The current cut is about {seconds} seconds. Choose MP4 to save it.',
  '已按{how}排列 {count} 个片段（约 {seconds} 秒）：{names}。在右侧预览里播放。': 'Ordered {count} clips by {how} (about {seconds} seconds): {names}. Play them in the preview.',
  '你给的顺序': 'your order',
  '名称顺序': 'name',
  '这些片段太短，不能再剪掉 {amount}。': 'These clips are too short to trim another {amount}.',
  '开头': 'the start',
  '结尾': 'the end',
  '已从 {count} 个片段的{where}剪掉约 {seconds} 秒，{skipped} 个太短已跳过。': 'Trimmed about {seconds} seconds from {where} of {count} clips. Skipped {skipped} that were too short.',
  '已从 {count} 个片段的{where}剪掉约 {seconds} 秒。': 'Trimmed about {seconds} seconds from {where} of {count} clips.',
  '这些片段已经不长于 {seconds} 秒。': 'These clips are already no longer than {seconds} seconds.',
  '已把 {count} 个片段收到最多 {seconds} 秒。': 'Capped {count} clips at {seconds} seconds.',
  '“{name}”还在素材库里，时间线上没有可删的片段。': '“{name}” is still in the media pool. There is no timeline clip to remove.',
  '已去掉“{name}”，后面的片段已靠拢。': 'Removed “{name}” and closed the gap.',
  '片段太短，放不下淡入淡出。': 'The clips are too short for a fade in and out.',
  '已为 {count} 个片段加上约 {seconds} 秒淡入淡出。': 'Added about {seconds} seconds of fade to {count} clips.',
  '这些画面片段没有可开关的原声。': 'These picture clips have no original audio to toggle.',
  '已静音 {count} 个片段。': 'Muted {count} clips.',
  '已恢复 {count} 个片段的声音。': 'Restored audio on {count} clips.',
  '已做成约 {seconds} 秒的短片，共 {count} 个片段：{names}。在预览里播放，再说「导出」。': 'Assembled a short of about {seconds} seconds from {count} clips: {names}. Play the preview, then say “Export”.',
  '这条指令还不能在本地执行。': 'This command cannot run locally yet.',
